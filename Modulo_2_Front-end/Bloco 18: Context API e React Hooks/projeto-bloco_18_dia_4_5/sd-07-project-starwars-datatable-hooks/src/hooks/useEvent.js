...
function useEvent() {
  ...
  const handlerClickSorted = ({ target }) => {
    setFilter(true);
    const { name, value } = target;
    setFilters({
      ...filters,
    ...
    const datas = !filter ? filterNumber : data;
    const zero = 0;
    const menosUm = -1;
    const { order } = filters;
  ...
  return [filtered, titles, option, names, filterByNumericValues,
    ...
    removeFilter, handlerClickSorted];
}
