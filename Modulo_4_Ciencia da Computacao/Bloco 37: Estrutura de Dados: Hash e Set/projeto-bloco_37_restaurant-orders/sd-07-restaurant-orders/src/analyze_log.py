...
def analyze_log(path_to_file):
    ...
    mariaFavorite = most_asked(order_list, 'maria')
    arnaldoTimes = times_asked(order_list, 'arnaldo', 'hamburguer')
    joaoPediu = never_asked(order_list, 'joao', products, 'product')
    joaoFoi = never_asked(order_list, 'joao', days, 'days_of_week')

    file = open("data/mkt_campaign.txt", "w")
    file.write(f"{mariaFavorite}\n")
    file.write(f"{arnaldoTimes}\n")
    file.write(f"{joaoPediu}\n")
    file.write(f"{joaoFoi}")
