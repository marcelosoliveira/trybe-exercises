# Exercício 2 - Google
# Uma certa empresa tem uma estrutura hierarquizada onde cada pessoa reporta a uma
# única outra pessoa. Cada pessoa tem um score que é o total de pessoas que estão
# abaixo dela, incluindo subordinados de seus subordinados, além dela própria.
# Isso significa que uma pessoa que não tem nenhuma equipe tem score 1. Uma pessoa
...
# Bônus: É muito comum em entrevistas que, quando a pessoa resolve o problema dentro
# do tempo, façam-se perguntas "follow-up" , que dificultam a questão. Os follow-ups
# abaixo são opcionais.

# Follow-ups:
# Caso a empresa precise fazer essa consulta frequentemente, como você pode tornar
# essas consultas mais eficientes? Como você pode guardar o resultado de consultas
# anteriores?
...
# Se o time estiver cheio, a nova contratada pode ser alocada na equipe de qualquer
# pessoa abaixo dela, não precisando ser na equipe imediatamente abaixo.
# Se você adicionar uma nova contratação à lista de subordinadas, a estrutura que
# você fez no follow-up 1 está desatualizada. Modifique suas funções para que essa
# estrutura seja atualizada junto com a adição de uma nova contratação. Faça isso
# sem rodar de novo a função score() e lembre-se de atualizar todos os scores desde
# a pessoa da presidência até a equipe que a nova contratação entrou.

# Solução:
# Em primeiro lugar, precisamos notar que o mapeamento de cada pessoa para seus
# subordinados pode ser representado como uma hash . Em segundo lugar, precisamos
# observar que a relação abaixo:
...
# Se você calcular para a pessoa 1, você concorda que teremos que calcular o score de
# todas as pessoas da empresa? O que podemos fazer é guardar os scores em uma
# outra hash para uso futuro. Poderíamos modificar a função score para receber
# essa estrutura e fazer a consulta de se aquele score já foi calculado antes.
# Mas essa estrutura auxiliar é parte do cálculo dos scores e não faz sentido que
# ela fique solta na main . Então vamos envelopar isso tudo dentro de uma classe.
...
# Vamos definir um exemplo para pensar: Se eu quiser adicionar a pessoa 8 na equipe
# de 4, com k=2?
...
# equipe e eu posso atribuir a nova contratada para ela. Ou seja, ao invés de tentar
# olhar em "largura" e tentar incluir em 5 e depois em 6, eu posso ir descendo na
# cadeia até achar um lugar que ela caiba, olhando em "profundidade": olho 4,
# depois 5, se 5 estivesse cheio, olho a 7.
...
# essa estrutura seja atualizada junto com a adição de uma nova contratação.
# Faça isso sem rodar de novo a função score() e lembre-se de atualizar todos os
# scores desde a pessoa da presidência até a equipe que a nova contratação entrou.

# Quando você adiciona uma pessoa, ela pode ser adicionada em qualquer equipe abaixo
# dela. Como você recebe o primeiro boss , para atualizar o score entre esse boss e o
# local que a pessoa foi efetivamente alocada, basta ir atualizando o score na medida
# em que você vai tentando uma nova equipe.
# A parte mais complicada é atualizar desde a presidência até o primeiro boss.
# A gente tem uma estrutura que nos diz quem é subordinado a quem, mas não temos um
# jeito imediato de consultar quem é chefe de quem. Então uma possível solução é, 
# a cada inserção, manter o controle da chefia imediata daquela pessoa. Na solução
# proposta abaixo, adicionamos a hash imediate_boss , que é atualizada antes de
# realmente tentarmos alocar a pessoa. A função add_person foi renomeada para
# fit_person e agora atualizada o score de cada boss que tentamos encaixar a nova
# contratada.
